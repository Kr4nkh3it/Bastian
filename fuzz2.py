...
def Find1(file,method,db,db2,word,url,port,prtcl,out,ext):
    if out == "1":
...
def Find2(file,method,word,url,port,prtcl,db,db2,out,ext):
    for directory in db:
        if prtcl == "http":
            addr = http.client.HTTPConnection(url,port)
        elif prtcl == "https":
        ...
        if status_code in db2:
            ...
            if out == "1":
                ...
                file.write(f"dir {directory}/{word}")
        ...
        if status_code2 in db2:
            ...
            if out == "1":
                ...
                file.write(f"file {directory}/{word}{ext}")
...
def threaded(file,prtcl,port,method,list_,url,out,ext,errors):
    while True:
        word = list_.get()
        try:
            Find1(file,method,database,error_codes,word,url,port,prtcl,out,ext)
        except TypeError:
            continue
            ...
            print("[SSL Error]: Try a different port")
            break
        list_.task_done()
        
#runs find2 through each word in the wordlist
def threaded2(file,prtcl,port,method,list2,url,out,ext,errors):
    while True:
        word = list2.get()
        try:
            Find2(file,method,word,url,port,prtcl,database,error_codes,out,ext)
        except TypeError:
            continue
        except http.client.RemoteDisconnected or http.client.ResponseNotReady:
            errors+1
