...
def Find1(db,db2,word,url,out,ext):
    if out == "1":
...
def Find2(word,url,db,db2,out,ext):
    for directory in db:
...
def threaded(list_,url,out,ext,errors):
    while True:
        word = list_.get()
        try:
            Find1(database,error_codes,word,url,out,ext)
        except TypeError:
            continue
...
def threaded2(list2,url,out,ext,errors):
    while True:
        word = list2.get()
        try:
            Find2(word,url,database,error_codes,out,ext)
        except TypeError:
            continue
        except ConnectionAbortedError or ConnectionResetError:
            errors+1
