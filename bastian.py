...
for thread in range(thrds):
    if errors <= 40:
        t = threading.Thread(target=threaded,args = [sock,list_,url,port,out,ext,errors])
        t.daemon = True
        t.start()
        if len(database) >=1:
            t2 = threading.Thread(target=threaded2,args = [sock,list2,url,port,out,ext,errors])
            t2.daemon = True
            t2.start()
    else:
